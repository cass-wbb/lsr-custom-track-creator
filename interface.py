...
def main():
    ...
    while not end:
        if input("Would you like to continue adding pieces to the track? y/n ") == "n":
            ...
            id = input("Enter the track piece ID that you want to add. Track piece IDs are listed in supported_track_pieces.txt. If you want to clear a spot, type clear. ")
            if id == "clear":
                place_piece.clear(lines)
            else:
                ...
                place_piece.place_other_part(x, y, orientation, id, elevation, biome_select, lines)
